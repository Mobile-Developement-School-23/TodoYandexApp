...
extension TodoListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row != data.count {
            ...
            string.addAttribute(NSAttributedString.Key.foregroundColor, value: AssetsColors.labelTertiary, range: NSRange(location: 0, length: string.length))
            string.addAttribute(NSAttributedString.Key.font, value: AssetsFonts.body, range: NSRange(location: 0, length: string.length))
