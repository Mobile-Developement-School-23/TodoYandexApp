...
extension TodoItem: CsvCompatible {
    var csv: String {
        return "\"\(id)\";\"\(text)\";\"\(done ? "true" : "false")\";\"\(importance != .basic ? importance.rawValue : "")\";\"\(deadline != nil ? String(deadline!.timeIntervalSince1970) : "")\";\"\(String(createdAt.timeIntervalSince1970))\";\"\(changedAt != nil ? String(changedAt!.timeIntervalSince1970) : "")\""
    }
    
    //TODO: Add values with quotes support
    ///Parse csv file, delimeter must be ";", the values must go in the order given by init, qutes (") in values must be replaced with (""")
